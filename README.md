...
+ PC側の機構データに反映して、脚の角度が可視化できるようにする
  + ワイヤーフレーム表示に反映する
    ...
    + 他のライブラリとの関係をクラス図にして、受信したデータを
      ...
      の形で値をセットして、アップデートして角度を決める...完了
      サーボのポイントから角度を割り出す関数を作成して、その角度をrobotにセットする...完了
+ PCからの指令で、手動/自動を切り替える機能を実装する
  + 手動モードはArduino側のダイヤル調整で調整する
  + 自動モードはPCから指令を受けて動かす
  + 初期の実装は、PCのスライダコントロールで移動する
  + 次に、PC側からある程度の制御能力を持たせる
  + PCの実装を十分に練ってから、Arduino側に移植する
+ M5Stack側をリモコンに改造して、リモコン機能を別ユニットに移行する
  + M5StackとWioをWifi接続してリモコン化する
